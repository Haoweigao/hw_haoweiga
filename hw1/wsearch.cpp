...
bool readGrid(istream& instream, vector<vector<char> >& grid )
{
  ...
  while(getline(instream,line)){
    ...
    else if(line.size()!=size || line.size()==0){
      return false;
    }
    vector<char> temp;
...
{
  /*** You complete ***/
  if(word.size()<=currWordIndex||currLoc.row<0||currLoc.col<0||currLoc.row>=grid.size()||currLoc.col>=grid[0].size()){
    return false;
  }

  else if((word.size()-1)==currWordIndex && word[currWordIndex]==grid[currLoc.row][currLoc.col]){
    return true;
  }
  else if(grid[currLoc.row][currLoc.col]!=word[currWordIndex]){
    return false;
  }
  
  else{
    cout<<delta.row<<delta.col;
    return findWordHelper(grid,Location((currLoc.row+delta.row),(currLoc.col+delta.col)),delta,word,++currWordIndex);
  }
}
